...
<template>
	<footer class="bg-slate-700 w-full z-20 py-5 md:py-8">
			...
			<div class="flex items-center justify-between">
				<router-link to="/" class="flex items-center space-x-2.5">
					<img
						...
						alt="footer-logo"
					/>
					<span class="text-2xl font-semibold text-white">VueShop</span>
				</router-link>
				<ul class="flex items-center mt-2 sm:mt-0 gap-x-2 md:gap-x-5">
					<span class="text-sm text-neutral-300 hidden sm:block">© Azimxan 2024.</span>
					<li v-for="(link, index) in socialLinks" :key="index">
						<a
							:href="link.href"
							class="text-white hover:text-blue-500 transition-all duration-200"
